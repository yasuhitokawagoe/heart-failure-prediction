...
def main():
    ...
    # 時系列分割
    splits = create_seasonal_splits(df, n_splits=50)
    ...
    # 再開機能: 既存の結果を読み込み
    results_file = f'{results_dir}/optimization_results.json'
    if os.path.exists(results_file):
        with open(results_file, 'r', encoding='utf-8') as f:
            cv_results = json.load(f)
        print(f"既存の結果を読み込みました: {len(cv_results)}個のFold完了")
    
    print(f"\n心不全特化の最適化による学習開始...")
    print(f"予想時間: 約6-8時間（50フォルド × 50回最適化）")
    print(f"進捗: 各Foldで進捗バーが表示されます")
    
    for fold, (X_train, y_train, X_test, y_test, feature_columns) in enumerate(splits, 1):
        # 既に完了しているFoldはスキップ
        if any(r['fold'] == fold for r in cv_results):
            print(f"Fold {fold} は既に完了済みです。スキップします。")
            continue
            
        fold_start_time = time.time()
        print(f"\n{'='*20} Fold {fold}/50 {'='*20}")
        print(f"進捗: {fold}/50 ({(fold/50)*100:.1f}%)")
        
        # ハイパーパラメータ最適化
        print(f"    心不全特化のハイパーパラメータ最適化開始...")
        best_params = optimize_hf_hyperparameters(X_train, y_train, X_test, y_test)
        
        # アンサンブルモデル作成
        print(f"    心不全特化のアンサンブルモデル作成...")
        ...
        print(f"標準評価: Precision={precision_std:.4f}, Recall={recall_std:.4f}, F1={f1_std:.4f}")
        print(f"F1最適化: Precision={precision_f1:.4f}, Recall={recall_f1:.4f}, F1={f1_opt:.4f} (閾値={threshold_f1:.3f})")
        print(f"PR最適化: Precision={precision_pr:.4f}, Recall={recall_pr:.4f} (閾値={threshold_pr:.3f})")
    ...
    # 最適化された指標の計算
    f1_optimized_scores = [r['f1_optimized'] for r in cv_results]
    precision_f1_scores = [r['precision_f1'] for r in cv_results]
    recall_f1_scores = [r['recall_f1'] for r in cv_results]
    
    precision_pr_scores = [r['precision_pr'] for r in cv_results]
    recall_pr_scores = [r['recall_pr'] for r in cv_results]
    
    total_time = time.time() - start_time
    
    print(f"\n{'='*50}")
    print(f"=== 心不全気象予測モデル 50回Fold最適化版 最終結果 ===")
    print(f"roc_auc: {np.mean([r['auc'] for r in cv_results]):.4f} ± {np.std([r['auc'] for r in cv_results]):.4f}")
    print(f"pr_auc: {np.mean([r['pr_auc'] for r in cv_results]):.4f} ± {np.std([r['pr_auc'] for r in cv_results]):.4f}")
    print(f"precision: {np.mean([r['precision_std'] for r in cv_results]):.4f} ± {np.std([r['precision_std'] for r in cv_results]):.4f}")
    print(f"recall: {np.mean([r['recall_std'] for r in cv_results]):.4f} ± {np.std([r['recall_std'] for r in cv_results]):.4f}")
    print(f"f1_score: {np.mean([r['f1_std'] for r in cv_results]):.4f} ± {np.std([r['f1_std'] for r in cv_results]):.4f}")
    
    print(f"\n=== 心不全特化の最適化された評価結果 ===")
    print(f"F1最適化 - F1-score: {np.mean(f1_optimized_scores):.4f} ± {np.std(f1_optimized_scores):.4f}")
    print(f"F1最適化 - Precision: {np.mean(precision_f1_scores):.4f} ± {np.std(precision_f1_scores):.4f}")
    print(f"F1最適化 - Recall: {np.mean(recall_f1_scores):.4f} ± {np.std(recall_f1_scores):.4f}")
    print(f"PR最適化 - Precision: {np.mean(precision_pr_scores):.4f} ± {np.std(precision_pr_scores):.4f}")
    print(f"PR最適化 - Recall: {np.mean(recall_pr_scores):.4f} ± {np.std(recall_pr_scores):.4f}")
    
    print(f"\n全体AUC: {overall_auc:.4f}")
    ...
    print(f"完了Fold数: {len(cv_results)}/50")
    ...
    # main()のfor foldループ内、アンサンブルモデル作成直後に追加
    save_dir = '心不全気象予測モデル_50回Fold最適化版_保存モデル'
    os.makedirs(save_dir, exist_ok=True)
    # LightGBM
    if 'lgb' in models:
        joblib.dump(models['lgb'], f'{save_dir}/lgb_model_fold{fold}.pkl')
    # XGBoost
    if 'xgb' in models:
        joblib.dump(models['xgb'], f'{save_dir}/xgb_model_fold{fold}.pkl')
    # CatBoost
    if 'cb' in models:
        joblib.dump(models['cb'], f'{save_dir}/cb_model_fold{fold}.pkl')
