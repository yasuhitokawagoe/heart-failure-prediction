...
def load_ensemble_model_and_data():
    ...
    try:
        ...
        # XGBoostモデル
        try:
            models['xgb'] = joblib.load('saved_models/xgb_model_latest.pkl')
        ...
        model_files = {
            'lgb': 'saved_models/lgb_model_latest.pkl',
            'cat': 'saved_models/cat_model_latest.pkl',
            'deep_nn': 'saved_models/deep_nn_model_latest.pkl',
            'attention_nn': 'saved_models/attention_nn_model_latest.pkl'
        }
...
def create_ensemble_shap_visualizations(X_scaled, y, ensemble_shap_values, explainer, 
    ...
    # 結果保存用ディレクトリ作成
    import os
    os.makedirs('ensemble_shap_results', exist_ok=True)
    ...
    plt.title('Ensemble SHAP Summary Plot - AMI Weather Model', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('ensemble_shap_results/ensemble_shap_summary_plot.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('Ensemble SHAP Feature Importance - AMI Weather Model', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('ensemble_shap_results/ensemble_shap_feature_importance.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if individual_predictions:
        ...
        if len(model_names) > 1:
            ...
            plt.tight_layout()
            plt.savefig('ensemble_shap_results/individual_model_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
            print("✓ 個別モデル比較保存完了")
    ...
    for i, feature in enumerate(top_features):
        ...
        plt.tight_layout()
        plt.savefig(f'ensemble_shap_results/ensemble_shap_dependence_{feature}.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def create_ensemble_shap_report(X_scaled, y, ensemble_shap_values, individual_predictions, feature_columns):
    ...
    report = {
        ...
        'model_type': 'Ensemble (XGBoost, LightGBM, CatBoost, Neural Networks)',
    ...
    # JSONファイルとして保存
    import json
    with open('ensemble_shap_results/ensemble_shap_analysis_report.json', 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2, default=str)
    
    # Markdownファイルとして保存
    with open('ensemble_shap_results/ensemble_shap_analysis_report.md', 'w', encoding='utf-8') as f:
        f.write('# AMI Ensemble Weather Model SHAP Analysis Report\n\n')
...
def main():
    """メイン実行関数"""
    print("=== AMI Ensemble Weather Model SHAP Analysis ===")
    ...
    print("\n=== アンサンブルSHAP解析完了 ===")
    print("✓ 結果は 'ensemble_shap_results/' ディレクトリに保存されました")
