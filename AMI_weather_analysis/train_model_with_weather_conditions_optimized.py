...
def main():
    """メイン実行関数（天気概況を含む、最適化版）"""
    try:
        print("=== 天気概況を含むAMI予測モデル（最適化版） ===")
        ...
        for fold, (X_train, y_train, X_test, y_test, feature_columns) in enumerate(splits, 1):
            ...
            for n in [30, 50, 70, 100]:
                ...
                try:
                    ...
                    # 天気概況関連の重要度を確認
                    weather_importance = [f for f in reduced_features if '天気' in f or 'encoded' in f]
        ...
        for result in cv_results:
            weather_features = [f for f in result['selected_features'] if '天気' in f or 'encoded' in f]
        ...
        # モデルを保存
        joblib.dump(models, 'weather_conditions_optimized_models.pkl')
        joblib.dump(weather_encoders, 'weather_encoders_optimized.pkl')
        
        print(f"\nモデルを保存しました: weather_conditions_optimized_models.pkl")
        print(f"エンコーダーを保存しました: weather_encoders_optimized.pkl")
