...
def encode_simplified_weather_conditions(df):
    """天気概況を6分類に簡略化してOne-Hot Encodingで数値化"""
    # 天気分類(統合)を6分類に再マッピング
    weather_mapping = {
        '晴れ': '晴れ系',
        '快晴': '晴れ系',
    ...
    # One-Hot Encodingで6つの独立した変数に変換
    weather_dummies = pd.get_dummies(df['weather_simplified'], prefix='weather')
    
    # 元の列を削除してOne-Hot Encoding列を追加
    df = df.drop('weather_simplified', axis=1)
    df = pd.concat([df, weather_dummies], axis=1)
    
    print("=== One-Hot Encodingされた天気分類 ===")
    print(f"作成された特徴量: {list(weather_dummies.columns)}")
    
    # 各分類の出現回数を確認
    print("\n=== 各分類の出現回数 ===")
    for col in weather_dummies.columns:
        count = weather_dummies[col].sum()
        print(f"{col}: {count}回")
    
    return df, weather_dummies.columns.tolist()
...
def main():
    ...
    try:
        ...
        print("⚠️ One-Hot Encodingされた6分類天気概況を含む気象情報のみを使用したモデルです")
        
        # 天気概況を6分類に簡略化してOne-Hot Encodingで数値化
        print("\n=== 天気概況の6分類One-Hot Encoding ===")
        df, weather_features = encode_simplified_weather_conditions(df)
        ...
        print(f"使用特徴量数: {len(feature_columns)} (One-Hot Encodingされた6分類天気概況を含む)")
        
        # 天気概況関連の特徴量を確認
        weather_condition_features = [col for col in feature_columns if 'weather_' in col]
        ...
        # 既存の学習・評価処理の後にXGBoostモデル保存を追加
        # 例: models['xgb']がXGBoostモデル
        if 'xgb' in models:
            joblib.dump(models['xgb'], 'saved_models/xgb_model_latest.pkl')
            print('XGBoostモデルをsaved_models/xgb_model_latest.pklに保存しました')
