...
def create_individual_prediction_analysis(model, X_scaled, feature_names, df):
    """個別予測の詳細分析（LIMEの代替）"""
    print("=== 個別予測の詳細分析 ===")
    ...
    for i, idx in enumerate(sample_indices):
        ...
        for j, feat_idx in enumerate(top_indices):
            ...
            feature_value = sample_values[feat_idx]
            ...
            print(f"  {j+1}. {japanese_name}: {feature_value:.3f} (重要度: {importance:.4f})")
        ...
        top_values = [sample_values[i] for i in top_indices]
        ...
        plt.yticks(range(len(top_features)), [feature_japanese.get(f, f) for f in top_features])
        plt.xlabel('標準化された特徴量値', fontsize=12)
        plt.title(f'個別サンプル分析: サンプル {idx} (予測確率: {predictions[idx]:.3f})', 
                 fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3)
        
        # 重要度を重ねて表示
        ax2 = plt.twinx()
        ax2.plot(top_importance, range(len(top_features)), 'ko-', markersize=8, label='重要度')
        ax2.set_xlabel('特徴量重要度', fontsize=12)
        
        plt.tight_layout()
        plt.savefig(f'個別予測分析結果/individual_analysis_sample_{idx}.png', dpi=300, bbox_inches='tight')
        plt.close()

def create_feature_contribution_analysis(model, X_scaled, feature_names, df):
    """特徴量貢献度の分析"""
    print("=== 特徴量貢献度の分析 ===")
    ...
    for idx in sample_indices:
        ...
        for i, (feature, importance) in enumerate(zip(feature_names, feature_importance)):
            if feature not in feature_contribution_summary:
                feature_contribution_summary[feature] = []
            
            # 特徴量の値と重要度の積を貢献度とする
            contribution = sample_values[i] * importance
            feature_contribution_summary[feature].append(contribution)
    
    # 平均貢献度を計算
    avg_contribution = {}
    for feature, contributions in feature_contribution_summary.items():
        avg_contribution[feature] = np.mean(np.abs(contributions))
    
    # 上位10個の特徴量を選択
    top_features = sorted(avg_contribution.items(), key=lambda x: x[1], reverse=True)[:10]
    
    # 可視化
    plt.figure(figsize=(12, 8))
    features = [item[0] for item in top_features]
    contributions = [item[1] for item in top_features]
    
    plt.barh(range(len(features)), contributions, color='lightgreen', alpha=0.7)
    plt.yticks(range(len(features)), features)
    plt.xlabel('平均貢献度', fontsize=12)
    plt.title('特徴量貢献度ランキング', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('個別予測分析結果/feature_contribution_summary.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_case_studies(model, X_scaled, feature_names, df):
    """具体的なケーススタディ"""
    print("=== 具体的なケーススタディ ===")
    ...
    for case_name, idx in case_studies:
        ...
        for j, feat_idx in enumerate(top_indices):
            ...
            japanese_name = feature_japanese.get(feature_name, feature_name)
            print(f"  - {japanese_name}: {feature_value:.3f} (重要度: {importance:.4f})")

def create_detailed_report():
    """詳細な分析レポートを作成"""
    print("=== 詳細レポートを作成 ===")
    
    with open('個別予測分析結果/個別予測分析レポート.md', 'w', encoding='utf-8') as f:
        f.write('# 個別予測分析レポート\n\n')
        ...
        f.write('- 臨床現場での意思決定支援ツールとしての活用\n')
    
    print("詳細レポートを保存しました: 個別予測分析結果/個別予測分析レポート.md")

def main():
    """メイン実行関数"""
    try:
        import os
        os.makedirs('個別予測分析結果', exist_ok=True)
        ...
        # 1. 個別予測の詳細分析
        create_individual_prediction_analysis(model, X_scaled, feature_names, df)
        
        # 2. 特徴量貢献度の分析
        create_feature_contribution_analysis(model, X_scaled, feature_names, df)
        
        # 3. 具体的なケーススタディ
        create_case_studies(model, X_scaled, feature_names, df)
        
        # 4. 詳細レポート
        create_detailed_report()
        
        print("\n=== 個別予測分析完了 ===")
        print("結果は '個別予測分析結果/' ディレクトリに保存されました")
