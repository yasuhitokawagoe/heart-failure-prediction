...
def load_ami_data():
    """AMIデータを読み込み"""
    df = pd.read_csv('ami_weather_merged.csv')
    df['hospitalization_date'] = pd.to_datetime(df['hospitalization_date_ami'])
    df['hospitalization_count'] = df['people_ami']
    return df
...
def create_comprehensive_weather_flags(df):
    ...
    # 基本的な異常気象フラグ
    df['is_tropical_night'] = (df['min_temp_ami'] >= 25).astype(int)
    df['is_extremely_hot'] = (df['max_temp_ami'] >= 35).astype(int)
    df['is_hot_day'] = (df['max_temp_ami'] >= 30).astype(int)
    df['is_summer_day'] = (df['max_temp_ami'] >= 25).astype(int)
    df['is_winter_day'] = (df['min_temp_ami'] < 0).astype(int)
    df['is_freezing_day'] = (df['max_temp_ami'] < 0).astype(int)
    
    # より詳細な温度フラグ
    df['is_very_hot'] = (df['max_temp_ami'] >= 33).astype(int)
    df['is_extremely_cold'] = (df['min_temp_ami'] <= -5).astype(int)
    df['is_cold_day'] = (df['max_temp_ami'] <= 5).astype(int)
    df['is_mild_day'] = (df['avg_temp_ami'] >= 15) & (df['avg_temp_ami'] <= 25)
    
    # 湿度関連フラグ
    df['is_very_humid'] = (df['avg_humidity_ami'] >= 80).astype(int)
    df['is_dry_day'] = (df['avg_humidity_ami'] <= 30).astype(int)
    df['is_comfortable_humidity'] = (df['avg_humidity_ami'] >= 40) & (df['avg_humidity_ami'] <= 70)
    
    # 気圧関連フラグ
    df['is_low_pressure'] = (df['vapor_pressure_ami'] <= df['vapor_pressure_ami'].rolling(30).mean() - 5).astype(int)
    df['is_high_pressure'] = (df['vapor_pressure_ami'] >= df['vapor_pressure_ami'].rolling(30).mean() + 5).astype(int)
    df['is_pressure_stable'] = (df['vapor_pressure_ami'] >= df['vapor_pressure_ami'].rolling(30).mean() - 2) & (df['vapor_pressure_ami'] <= df['vapor_pressure_ami'].rolling(30).mean() + 2)
    
    # 気象変化フラグ
    df['is_temp_change_large'] = (np.abs(df['avg_temp_ami'] - df['avg_temp_ami'].shift(1)) >= 5).astype(int)
    df['is_humidity_change_large'] = (np.abs(df['avg_humidity_ami'] - df['avg_humidity_ami'].shift(1)) >= 20).astype(int)
    df['is_pressure_change_large'] = (np.abs(df['vapor_pressure_ami'] - df['vapor_pressure_ami'].shift(1)) >= 3).astype(int)
...
def create_weather_interaction_features(df):
    """気象相互作用特徴量を作成"""
    # 気象相互作用
    df['temp_humidity'] = df['avg_temp_ami'] * df['avg_humidity_ami']
    df['temp_pressure'] = df['avg_temp_ami'] * df['vapor_pressure_ami']
    df['temp_change'] = (df['avg_temp_ami'] - df['avg_temp_ami'].shift(1)).fillna(0)
    df['humidity_change'] = (df['avg_humidity_ami'] - df['avg_humidity_ami'].shift(1)).fillna(0)
    df['pressure_change'] = (df['vapor_pressure_ami'] - df['vapor_pressure_ami'].shift(1)).fillna(0)
    
    return df

def create_time_series_features(df):
    """時系列特徴量を作成"""
    weather_cols = ['avg_temp_ami', 'avg_humidity_ami', 'vapor_pressure_ami']
...
def select_features(df):
    """特徴量を選択"""
    exclude_cols = ['hospitalization_date', 'target', 'season', 'prefecture_name_ami', 'date', 
                   'hospitalization_count', 'people_ami']
