...
def check_temperature_features():
    """気温関連特徴量の重要度を詳細分析"""
    print("=== 気温関連特徴量の重要度分析 ===")
    ...
    # 気温関連特徴量の特定
    temperature_features = [col for col in feature_columns if 'temp' in col.lower()]
    print(f"\n=== 気温関連特徴量一覧（{len(temperature_features)}個） ===")
    for i, feature in enumerate(temperature_features, 1):
    ...
    # 1. 基本気温
    basic_temp = [f for f in temperature_features if f in ['avg_temp', 'max_temp', 'min_temp']]
    print("1. 基本気温:")
    for feature in basic_temp:
        print(f"   - {feature}")
    
    # 2. 気温の時系列特徴量
    temp_timeseries = [f for f in temperature_features if any(x in f for x in ['ma_', 'std_', 'lag_'])]
    print("\n2. 気温の時系列特徴量:")
    for feature in temp_timeseries:
        print(f"   - {feature}")
    
    # 3. 気温の変化率・変化量
    temp_change = [f for f in temperature_features if any(x in f for x in ['change_rate', 'change'])]
    print("\n3. 気温の変化率・変化量:")
    for feature in temp_change:
        print(f"   - {feature}")
    
    # 4. 気温の季節性重み付け
    temp_seasonal = [f for f in temperature_features if 'seasonal_weighted' in f]
    print("\n4. 気温の季節性重み付け:")
    for feature in temp_seasonal:
        print(f"   - {feature}")
    
    # 5. 気温関連の相互作用
    temp_interaction = [f for f in temperature_features if any(x in f for x in ['temp_humidity', 'temp_pressure'])]
    print("\n5. 気温関連の相互作用:")
    for feature in temp_interaction:
        print(f"   - {feature}")
    
    # 6. 気温関連の異常気象フラグ
    temp_flags = [f for f in temperature_features if f.startswith('is_')]
    print("\n6. 気温関連の異常気象フラグ:")
    for feature in temp_flags:
        print(f"   - {feature}")
    ...
    # 気温関連特徴量の重要度確認
    print(f"\n=== 気温関連特徴量の重要度（選択された65個中） ===")
    temp_features_in_selected = [f for f in reduced_features if f in temperature_features]
    for feature in temp_features_in_selected:
        if feature in feature_columns:
            idx = feature_columns.index(feature)
            importance = importances[idx]
            rank = list(importances).index(importance) + 1
            print(f"{feature}: {importance:.4f} (順位: {rank}/{len(importances)})")
    ...
    for feature in temperature_features:
        if feature in feature_columns:
            idx = feature_columns.index(feature)
            importance = importances[idx]
            temp_importances.append(importance)
    
    if temp_importances:
        print(f"気温関連特徴量数: {len(temp_importances)}")
        print(f"平均重要度: {np.mean(temp_importances):.4f}")
        print(f"最大重要度: {np.max(temp_importances):.4f}")
        print(f"最小重要度: {np.min(temp_importances):.4f}")
        print(f"重要度の標準偏差: {np.std(temp_importances):.4f}")
