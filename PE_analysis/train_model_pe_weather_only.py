...
def load_processed_data():
    ...
    # 統合データを読み込み
    df = pd.read_csv('pe_weather_merged.csv')
    df['date'] = pd.to_datetime(df['date'])
    
    # PEデータを正しく設定
    df['hospitalization_count'] = df['people_pe']
    
    print(f"データ期間: {df['date'].min()} から {df['date'].max()}")
    print(f"総データ数: {len(df)}")
    print(f"PE発生総数: {df['people_pe'].sum()}")
    print(f"平均PE発生数/日: {df['people_pe'].mean():.2f}")
...
def prepare_data_for_training(df, train_dates, test_dates):
    ...
    # 特徴量とターゲットを分離
    exclude_cols = ['hospitalization_date', 'target', 'season', 'prefecture_name', 'date', 'hospitalization_count', 'people_pe', 'people_weather']
