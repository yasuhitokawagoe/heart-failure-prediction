...
def create_comprehensive_weather_flags(df):
    ...
    # より詳細な温度フラグ
    df['is_very_hot'] = (df['max_temp'] >= 33).astype(int)
    df['is_extremely_cold'] = (df['min_temp'] <= -5).astype(int)
    df['is_cold_day'] = (df['max_temp'] <= 5).astype(int)
    df['is_mild_day'] = (df['avg_temp'] >= 15) & (df['avg_temp'] <= 25)
    
    # 湿度関連フラグ
    df['is_very_humid'] = (df['avg_humidity'] >= 80).astype(int)
    df['is_dry_day'] = (df['avg_humidity'] <= 30).astype(int)
    df['is_comfortable_humidity'] = (df['avg_humidity'] >= 40) & (df['avg_humidity'] <= 70)
    
    # 気圧関連フラグ
    df['is_low_pressure'] = (df['vapor_pressure'] <= df['vapor_pressure'].rolling(30).mean() - 5).astype(int)
    df['is_high_pressure'] = (df['vapor_pressure'] >= df['vapor_pressure'].rolling(30).mean() + 5).astype(int)
    df['is_pressure_stable'] = (df['vapor_pressure'] >= df['vapor_pressure'].rolling(30).mean() - 2) & (df['vapor_pressure'] <= df['vapor_pressure'].rolling(30).mean() + 2)
    
    # 気象変化フラグ
    df['is_temp_change_large'] = (np.abs(df['avg_temp'] - df['avg_temp'].shift(1)) >= 5).astype(int)
    df['is_humidity_change_large'] = (np.abs(df['avg_humidity'] - df['avg_humidity'].shift(1)) >= 20).astype(int)
    df['is_pressure_change_large'] = (np.abs(df['vapor_pressure'] - df['vapor_pressure'].shift(1)) >= 3).astype(int)
