...
def load_processed_data():
    ...
    # 東京全体データを正しく設定
    df['hospitalization_count'] = df['people_tokyo']
    
    print(f"データ期間: {df['date'].min()} から {df['date'].max()}")
    print(f"総データ数: {len(df)}")
    print(f"東京全体発生総数: {df['people_tokyo'].sum()}")
    print(f"平均東京全体発生数/日: {df['people_tokyo'].mean():.2f}")
...
def prepare_data_for_training(df, train_dates, test_dates):
    ...
    # 特徴量とターゲットを分離
    exclude_cols = ['hospitalization_date', 'target', 'season', 'prefecture_name', 'date', 'hospitalization_count', 'people_tokyo', 'people_weather']
