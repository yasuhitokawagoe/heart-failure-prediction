...
def analyze_pressure_features():
    """気圧関連特徴量の重要度を分析"""
    print("=== VT/VF 気圧関連特徴量重要度分析 ===")
    ...
    # 気圧関連特徴量を抽出
    pressure_features = importance_df[importance_df['feature'].str.contains('pressure|vapor')]
    
    print(f"\n=== 気圧関連特徴量重要度 ===")
    print(f"全特徴量数: {len(feature_columns)}")
    print(f"気圧関連特徴量数: {len(pressure_features)}")
    
    if len(pressure_features) > 0:
        print("\n気圧関連特徴量の重要度ランキング:")
        for i, row in pressure_features.iterrows():
            rank = importance_df[importance_df['feature'] == row['feature']].index[0] + 1
            print(f"{rank:3d}位: {row['feature']:<35} - {row['importance']:.4f}")
    else:
        print("気圧関連特徴量が見つかりませんでした")
    
    # 上位20位内の気圧関連特徴量を確認
    top_20 = importance_df.head(20)
    top_20_pressure_features = top_20[top_20['feature'].str.contains('pressure|vapor')]
    
    print(f"\n=== 上位20位内の気圧関連特徴量 ===")
    if len(top_20_pressure_features) > 0:
        for i, row in top_20_pressure_features.iterrows():
            rank = top_20[top_20['feature'] == row['feature']].index[0] + 1
            print(f"{rank:2d}位: {row['feature']:<35} - {row['importance']:.4f}")
    else:
        print("上位20位内に気圧関連特徴量はありません")
    
    # 全体的な重要度の分布
    print(f"\n=== 重要度統計 ===")
    print(f"全特徴量の平均重要度: {importance_df['importance'].mean():.4f}")
    if len(pressure_features) > 0:
        print(f"気圧関連特徴量の平均重要度: {pressure_features['importance'].mean():.4f}")
        print(f"気圧関連特徴量の最高重要度: {pressure_features['importance'].max():.4f}")
        print(f"気圧関連特徴量の最低重要度: {pressure_features['importance'].min():.4f}")
    ...
    if len(pressure_features) > 0:
        ...
        # 水蒸気圧関連
        vapor_features = pressure_features[pressure_features['feature'].str.contains('vapor')]
        if len(vapor_features) > 0:
            print(f"\n水蒸気圧関連特徴量 ({len(vapor_features)}個):")
            for i, row in vapor_features.iterrows():
                rank = importance_df[importance_df['feature'] == row['feature']].index[0] + 1
                print(f"  {rank:3d}位: {row['feature']:<35} - {row['importance']:.4f}")
        
        # 気圧変化率関連
        change_rate_features = pressure_features[pressure_features['feature'].str.contains('change_rate')]
        if len(change_rate_features) > 0:
            print(f"\n気圧変化率関連特徴量 ({len(change_rate_features)}個):")
            for i, row in change_rate_features.iterrows():
                rank = importance_df[importance_df['feature'] == row['feature']].index[0] + 1
                print(f"  {rank:3d}位: {row['feature']:<35} - {row['importance']:.4f}")
        
        # 気圧移動平均関連
        ma_features = pressure_features[pressure_features['feature'].str.contains('ma_')]
        if len(ma_features) > 0:
            print(f"\n気圧移動平均関連特徴量 ({len(ma_features)}個):")
            for i, row in ma_features.iterrows():
                rank = importance_df[importance_df['feature'] == row['feature']].index[0] + 1
                print(f"  {rank:3d}位: {row['feature']:<35} - {row['importance']:.4f}")
        
        # 気圧標準偏差関連
        std_features = pressure_features[pressure_features['feature'].str.contains('std_')]
        if len(std_features) > 0:
            print(f"\n気圧標準偏差関連特徴量 ({len(std_features)}個):")
            for i, row in std_features.iterrows():
                rank = importance_df[importance_df['feature'] == row['feature']].index[0] + 1
                print(f"  {rank:3d}位: {row['feature']:<35} - {row['importance']:.4f}")
    
    return importance_df, pressure_features

if __name__ == "__main__":
    importance_df, pressure_features = analyze_pressure_features() 