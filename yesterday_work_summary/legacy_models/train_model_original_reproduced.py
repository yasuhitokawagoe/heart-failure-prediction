...
def save_detailed_results(cv_results, feature_importance=None, avg_auc=None):
    ...
    # 結果をJSONファイルとして保存
    with open(f'results_original_reproduced/detailed_metrics{auc_suffix}.json', 'w') as f:
        json.dump(overall_metrics, f, indent=4)
    # Fold別の結果をMarkdownファイルとして保存
    with open(f'results_original_reproduced/fold_results{auc_suffix}.md', 'w') as f:
        f.write('# モデル評価結果\n\n')
        # 全体の結果
        f.write('## 全体の性能\n\n')
        f.write('| 指標 | 平均 ± 標準偏差 |\n')
    ...
    if feature_importance is not None:
        ...
        plt.title('特徴量の重要度')
        plt.tight_layout()
        plt.savefig(f'results_original_reproduced/feature_importance{auc_suffix}.png')
        plt.close()
        # 追加: 重要度をCSVでも保存
        feature_importance.to_csv(f'results_original_reproduced/feature_importance{auc_suffix}.csv', index=False)

def save_modelwise_metrics(modelwise_metrics, avg_auc=None):
    os.makedirs('results_original_reproduced', exist_ok=True)
    ...
    # JSON保存
    with open(f'results_original_reproduced/modelwise_metrics{auc_suffix}.json', 'w') as f:
        json.dump(modelwise_metrics, f, indent=4, ensure_ascii=False)
    # Markdown保存
    with open(f'results_original_reproduced/modelwise_metrics{auc_suffix}.md', 'w') as f:
        f.write('# 各モデルごとの評価指標\n\n')
...
def main():
    ...
    try:
        ...
        for fold, (X_train, y_train, X_test, y_test, feature_columns) in enumerate(splits, 1):
            ...
            # 性能曲線のプロット
            plot_model_performance(y_test, ensemble_pred, fold, avg_auc=np.mean([r['roc_auc'] for r in cv_results]))
        ...
        # 詳細な結果の保存（AUC付きファイル名）
        save_detailed_results(cv_results, feature_importance, avg_auc)
        # 各モデルごとの評価指標を保存（AUC付きファイル名）
        save_modelwise_metrics(modelwise_metrics, avg_auc)
