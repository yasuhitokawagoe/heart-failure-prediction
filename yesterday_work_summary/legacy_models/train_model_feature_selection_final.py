...
def prepare_data_for_training(df, train_dates, test_dates, feature_creation_func):
    ...
    # ターゲット変数を分割後に作成（リーク防止）
    # 訓練データの75%タイルを使用
    train_threshold = train_data['hospitalization_count'].quantile(0.75)
    train_data['target'] = (train_data['hospitalization_count'] >= train_threshold).astype(int)
    test_data['target'] = (test_data['hospitalization_count'] >= train_threshold).astype(int)
