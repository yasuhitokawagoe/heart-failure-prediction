...
def main():
    ...
    try:
        ...
        # 基準モデル用：日付関連の特徴量のみ作成（新規追加分は除外）
        df = create_date_features_baseline(df)
        
        # 基準モデル用：異常気象フラグ、インタラクション、時系列特徴量は除外
...
def create_date_features_baseline(df):
    """基準モデル用：日付関連の特徴量のみ作成（新規追加分は除外）"""
    df['year'] = df['hospitalization_date'].dt.year
    df['month'] = df['hospitalization_date'].dt.month
    df['day'] = df['hospitalization_date'].dt.day
...
def save_detailed_results(cv_results, feature_importance=None):
    """詳細な結果を保存する関数（基準モデル用：results_baseline/に保存）"""
    ...
    # 結果をJSONファイルとして保存
    with open('results_baseline/detailed_metrics.json', 'w') as f:
        json.dump(overall_metrics, f, indent=4)
    # Fold別の結果をMarkdownファイルとして保存
    with open('results_baseline/fold_results.md', 'w') as f:
        f.write('# 基準モデル評価結果\n\n')
        # 全体の結果
        f.write('## 全体の性能\n\n')
        f.write('| 指標 | 平均 ± 標準偏差 |\n')
    ...
    if feature_importance is not None:
        ...
        plt.title('基準モデル：特徴量の重要度')
        plt.tight_layout()
        plt.savefig('results_baseline/feature_importance.png')
        plt.close()
        # 追加: 重要度をCSVでも保存
        feature_importance.to_csv('results_baseline/feature_importance.csv', index=False)

def save_modelwise_metrics(modelwise_metrics):
    """各モデルごとの評価指標を保存（基準モデル用：results_baseline/に保存）"""
    os.makedirs('results_baseline', exist_ok=True)
    # JSON保存
    with open('results_baseline/modelwise_metrics.json', 'w') as f:
        json.dump(modelwise_metrics, f, indent=4, ensure_ascii=False)
    # Markdown保存
    with open('results_baseline/modelwise_metrics.md', 'w') as f:
        f.write('# 基準モデル：各モデルごとの評価指標\n\n')
        f.write('| Fold | Model | ROC-AUC | PR-AUC | Precision | Recall | F1 | Specificity |\n')
        f.write('|------|-------|---------|--------|-----------|--------|----|-------------|\n')
        for m in modelwise_metrics:
            f.write(f"| {m['fold']} | {m['model']} | {m['roc_auc']:.4f} | {m['pr_auc']:.4f} | {m['precision']:.4f} | {m['recall']:.4f} | {m['f1_score']:.4f} | {m['specificity']:.4f} |\n")
...
def plot_model_performance(y_true, y_pred_proba, fold_num=None):
    """モデルの性能を可視化する関数（基準モデル用：results_baseline/に保存）"""
    # ROC曲線
    fpr, tpr, _ = roc_curve(y_true, y_pred_proba)
    roc_auc = roc_auc_score(y_true, y_pred_proba)
    plt.figure(figsize=(12, 5))
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('基準モデル：Receiver Operating Characteristic (ROC)')
    plt.legend(loc="lower right")
    # PR曲線のプロット
    precision, recall, _ = precision_recall_curve(y_true, y_pred_proba)
    pr_auc = average_precision_score(y_true, y_pred_proba)
    ...
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('基準モデル：Precision-Recall Curve')
    plt.legend(loc="lower left")
    plt.tight_layout()
    # 保存するファイル名にfold番号を含める
    fold_str = f'_fold_{fold_num}' if fold_num is not None else ''
    plt.savefig(f'results_baseline/performance_curves{fold_str}.png')
    plt.close()
